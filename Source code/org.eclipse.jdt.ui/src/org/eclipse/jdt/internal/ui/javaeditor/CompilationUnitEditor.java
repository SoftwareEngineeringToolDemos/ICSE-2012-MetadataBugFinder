...
public class CompilationUnitEditor extends JavaEditor implements IJavaReconcilingListener {
	...
	private class ExitPolicy implements IExitPolicy {
		...
		/*
		 * @see
		 * org.eclipse.jdt.internal.ui.text.link.LinkedPositionUI.ExitPolicy#doExit(org.eclipse.jdt
		 * .internal.ui.text.link.LinkedPositionManager, org.eclipse.swt.events.VerifyEvent, int, int)
		 */
	...
	private class BracketInserter implements VerifyKeyListener, ILinkedModeListener {
		...
		/*
		 * @see org.eclipse.jface.text.link.ILinkedModeListener#left(org.eclipse.jface.text.link.
		 * LinkedModeModel, int)
		 */
		...
		/*
		 * @see org.eclipse.jface.text.link.ILinkedModeListener#suspend(org.eclipse.jface.text.link.
		 * LinkedModeModel)
		 */
		...
		/*
		 * @see org.eclipse.jface.text.link.ILinkedModeListener#resume(org.eclipse.jface.text.link.
		 * LinkedModeModel, int)
		 */
	 ...
	 * <p>
	 * XXX remove once the underlying problem (https://bugs.eclipse.org/bugs/show_bug.cgi?id=66176)
	 * is solved.
	 * </p>
	 */
	...
	protected void createActions()
	{
		...
		action =
				new TextOperationAction(JavaEditorMessages.getBundleForConstructedKeys(), "Uncomment.", this, ITextOperationTarget.STRIP_PREFIX); //$NON-NLS-1$
		...
		action =
				new TextOperationAction(JavaEditorMessages.getBundleForConstructedKeys(), "Format.", this, ISourceViewer.FORMAT); //$NON-NLS-1$
		...
		action =
				new AddBlockCommentAction(JavaEditorMessages.getBundleForConstructedKeys(), "AddBlockComment.", this); //$NON-NLS-1$
		...
		action =
				new RemoveBlockCommentAction(JavaEditorMessages.getBundleForConstructedKeys(), "RemoveBlockComment.", this); //$NON-NLS-1$
		...
		JavaMoveLinesAction[] moveLinesActions =
				JavaMoveLinesAction.createMoveCopyActionSet(JavaEditorMessages.getBundleForConstructedKeys(), this);
		...
		fCorrectionCommands = new CorrectionCommandInstaller(); // allow shortcuts for quick
																					// fix/assist
	...
	protected IJavaElement getCorrespondingElement(IJavaElement element)
	{
		...
		// Note that the previous code could result in a reconcile as side effect. Should check if
		// that
		// is still required.
		return element;
	}
	...
	// List<String> beforeFileContents = new ArrayList<String>();
	// List<String> afterFileContents = new ArrayList<String>();
	// int linenumber = -1;
	...
	public void doSave(IProgressMonitor progressMonitor)
	{
		...
		if (p.isDeleted(getEditorInput())) {
			...
			String refactoredFileName = Comm.getGlobalParm("target") + unit.getPath().toFile().getAbsolutePath();
			Open.beforeFileContents = UtilFile.fileRead2List(refactoredFileName);
			...
			if (unit != null) {
				...
				// * ------- smk mksong
				// System.out.println("[DBG] doSave: ");
				Open.afterFileContents = UtilFile.fileRead2List(refactoredFileName);
				// linenumber = UtilFile.fileCompare(beforeFileContents, afterFileContents);
				// Open.lineNumber = linenumber;
				// Open.beforeFileContents = beforeFileContents;
				// Open.afterFileContents = afterFileContents;
	 ...
	 * @METHOD
	 */
	private void checkMetadataInvariants(ICompilationUnit unit)
	{
		String refactoredFileName = Comm.getGlobalParm("target") + unit.getPath().toFile().getAbsolutePath();
		// System.out.println("[DBG] filename: " + refactoredFileName);

		// List<String> fileContents = UtilFile.fileRead2List(refactoredFileName);
		// UtilPrint.printArrayList(fileContents);
		...
		if (metadata.equals(STR.parm_annotation)) {
			...
			if (pattern.startsWith(STR.pbse_ptrn_match)) {
				if (pattern.contains(STR.var_method) && pattern.contains(STR.relation_modifier)) {
					Boolean result = checkMatchWithMethod(refactoredFileName, pattern);
					if (result != null)
						UtilPrint.displayResult(result, refactoredFileName);
				}
			}
			else if (pattern.startsWith(STR.pbse_ptrn_contains)) {
				if (pattern.contains(STR.var_method) && pattern.contains(STR.var_superclass)) {
					Boolean result = checkContainsWithMethod(refactoredFileName, pattern);
					if (result != null)
						UtilPrint.displayResult(result, refactoredFileName);
				}
			}
	 ...
	 * @METHOD
	 */
	private Boolean checkContainsWithMethod(String refactoredFileName, String pattern)
	{
		Object isChecked = null;
		String[] patternPair = UtilStr.getStrBetween(pattern, "(", ")").split(",");
		String annotation = Comm.getParm("relation").split(",")[2].trim();

		String _pair1 = patternPair[0].trim();
		String _pair2 = patternPair[1].trim();

		if (_pair1.equals(STR.var_method) && _pair2.equals(STR.var_superclass)) {

			List<AnnotatedMethodDecl> methodList = UtilAST.getMethodList(refactoredFileName);

			for (int i = 0; i < methodList.size(); i++) {
				AnnotatedMethodDecl elem = methodList.get(i);

				boolean checkAnnotation1 = elem.markerAnnotationList.contains(annotation);
				boolean checkAnnotation2 = elem.normalAnnotationList.contains(annotation);
				boolean checkSuperClass3 = (elem.superclazz != null && !elem.superclazz.isEmpty());
				Boolean hasSuperMethod4 = hasSuperClazzMethod(elem.superclazz, elem.name, elem.parm);

				isChecked = new Object();
				if (!checkAnnotation1 && !checkAnnotation2 && !checkSuperClass3)
					isChecked = null;

				if (checkSuperClass3 && hasSuperMethod4 != null && hasSuperMethod4) {
					if (checkAnnotation1 || checkAnnotation2)
						return true;
					else
						return false;
				}
				...
				if (checkAnnotation1 || checkAnnotation2) {
					if (hasSuperMethod4 != null && hasSuperMethod4)
						return true;
					else
						return false;
				}

				// if ((checkSuperClass3 && checkAnnotation1) || (checkSuperClass3 && checkAnnotation2))
				// {
				// if (hasSuperMethod4 != null)
				// return hasSuperMethod4;
				// }
			}
		}
		if (isChecked == null)
	 ...
	 * @METHOD
	 */
	private Boolean hasSuperClazzMethod(String superclazz, String name, String parm)
	{
		// System.out.println("[DBG]" + superclazz);
		// System.out.println("[DBG]" + name);
		// System.out.println("[DBG]" + parm);

		List<String> javaFileList = //
				UtilDirScan.getResultViaStr(Comm.getGlobalParm("target"), ".java");
		String targetJavaFile = null;
		for (int i = 0; i < javaFileList.size(); i++) {
			String elem = javaFileList.get(i);
			if (elem.contains(superclazz)) {
				targetJavaFile = elem;
				break;
			}
		}
		...
		List<AnnotatedMethodDecl> methodList = UtilAST.getMethodList(targetJavaFile);
		for (int i = 0; i < methodList.size(); i++) {
			AnnotatedMethodDecl elem = methodList.get(i);
			if (elem.name.equals(name) && elem.parm.equals(parm))
				return true;
		}
		return false;
	}
	 ...
	 * @METHOD
	 */
	Boolean checkMatchWithMethod(String refactoredFileName, String pattern)
	{
		Object isChecked = null;
		String[] patternPair = UtilStr.getStrBetween(pattern, "(", ")").split(",");
		String annotation = Comm.getParm("relation").split(",")[2].trim();
		...
		for (int i = 0; i < methodList.size(); i++) {
			...
			if (patternPair[0].trim().contains(STR.relation_modifier)) {
				boolean check1 = elem.modifier.equalsIgnoreCase(patternPair[1].trim());
				boolean check2 = elem.markerAnnotationList.contains(annotation);
				boolean check3 = elem.normalAnnotationList.contains(annotation);
	...
	// /** @METHOD */
	// void displayResult(boolean result, String filename)
	// {
	// System.out.println("------------------------------------------");
	// if (result) {
	...
	// System.out.println("[DBG] Changed (line number: " + linenumber + "): ");
	// String oldLine = Open.beforeFileContents.get(Open.lineNumber);
	// String newLine = Open.afterFileContents.get(Open.lineNumber);
	// System.out.println("[DBG] < " + oldLine);
	// System.out.println("[DBG] ---");
	// System.out.println("[DBG] >" + newLine);
	// }
	// System.out.println("------------------------------------------");
	// }
	...
	private void openSaveListenerWarningDialog(String title, String message, CoreException exception)
	{
		...
		final boolean hasProjectSettings =
				project != null && CleanUpPreferenceUtil.hasSettingsInScope(new ProjectScope(project));
		if (exception.getStatus().getCode() == IJavaStatusConstants.EDITOR_POST_SAVE_NOTIFICATION) {
			...
			if (hasProjectSettings)
				linkText =
						JavaEditorMessages.CompilationUnitEditor_error_saving_editedLines_calculation_property_link;
			else
		...
		ErrorDialog dialog = new ErrorDialog(getSite().getShell(), title, message, status, mask) {
			protected Control createMessageArea(Composite parent)
			{
				...
				// Panic code: use 'parent' instead of 'result' in case super implementation changes in
				// the future
	...
	public void createPartControl(Composite parent)
	{
		...
		boolean closeAngularBrackets =
				JavaCore.VERSION_1_5.compareTo(preferenceStore.getString(JavaCore.COMPILER_SOURCE)) <= 0;
		...
		try {
			...
			if (asv != null) {
				...
				if (JavaCore.COMPILER_SOURCE.equals(p)) {
					boolean closeAngularBrackets =
							JavaCore.VERSION_1_5.compareTo(getPreferenceStore().getString(p)) <= 0;
					fBracketInserter.setCloseAngularBracketsEnabled(closeAngularBrackets);
				}
	...
	/*
	 * @see
	 * org.eclipse.jdt.internal.ui.javaeditor.JavaEditor#createJavaSourceViewer(org.eclipse.swt.widgets
