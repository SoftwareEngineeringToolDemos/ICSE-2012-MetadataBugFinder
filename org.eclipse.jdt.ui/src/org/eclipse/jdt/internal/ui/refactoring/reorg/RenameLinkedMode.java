...
public class RenameLinkedMode {
	...
	private class ExitPolicy extends DeleteBlockingExitPolicy {
		...
		{
			fShowPreview =
					(event.stateMask & SWT.CTRL) != 0 && (event.character == SWT.CR || event.character == SWT.LF);
		...
		try {
			...
			for (int i = 0; i < sameNodes.length; i++) {
				...
				LinkedPosition linkedPosition =
						new LinkedPosition(document, elem.getStartPosition(), elem.getLength(), i);
				if (i == 0)
			...
			viewer.setSelectedRange(fOriginalSelection.x, fOriginalSelection.y); // by default, full
																										// word is selected;
																										// restore original
																										// selection
	...
	{
		// *------ smk, mksong source start
		String absolutePath = getCompilationUnit().getPath().toFile().getAbsolutePath();
		Open.setRefactFileName(absolutePath);
		// System.out.println("[DBG] RenameLinkedMode.java: " + absolutePath);
		// *------ smk, mksong source end

		Open.beforeFileContents = //
				UtilFile.fileRead2List(ParameterHelper.getGlobalParm("target") + absolutePath);
	...
	private RenameJavaElementDescriptor createRenameDescriptor(IJavaElement javaElement, String newName) throws JavaModelException
	{
		...
		RenameJavaElementDescriptor descriptor =
				(RenameJavaElementDescriptor) RefactoringCore.getRefactoringContribution(contributionId).createDescriptor();
